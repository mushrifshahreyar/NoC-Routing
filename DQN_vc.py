...
def update_replay_memory(replay_memory, my_id, dest_id, prev_router_id, prev_action, queueing_delay, done, is_dead, free_vcs):

    transition = [my_id, dest_id, prev_router_id, prev_action, queueing_delay, done, is_dead, free_vcs]
...
def train(model, target_model, replay_memory, target_update_counter):
    ...
    current_states = np.array([vectorize(transition[2], transition[1], transition[7]) for transition in minibatch])
    ...
    current_dead_states = np.array([vectorize(transition[0], transition[1], transition[7]) for transition in minibatch])
    ...
    new_current_states = np.array([vectorize(transition[0], transition[1], transition[7]) for transition in minibatch])
    ...
    for index, (my_id, dest_id, prev_router_id, prev_action, reward, done, is_dead, free_vcs) in enumerate(minibatch):
        ...
        if not is_dead:
            ...
            X.append(vectorize(prev_router_id, dest_id, free_vcs))
            y.append(current_qs)
        else:
            new_q = -10
            ...
            X.append(vectorize(my_id, dest_id, free_vcs))
            y.append(current_qs)
...
def get_qs(model, my_id, dest_id, free_vcs):

    #state = oneHotEncode(my_id, dest_id)
    #print('Size:', len(free_vcs))
    state = vectorize(my_id, dest_id, free_vcs)
    #actions = model.predict(np.array(state).reshape(-1, NSTATES))
    actions = model.predict(np.array(state).reshape(-1, 6))
    ...
    while(1):
     ...
     #   print('Free vcss:', free_vcs)
        ...
        if generate_model_action:
            m, action = get_qs(m, my_id, dest_id, free_vcs)
            ...
            rm = update_replay_memory(rm, my_id, dest_id, prev_router_id, action, queueing_delay, done, is_dead, free_vcs)
        ...
        if my_id != src_id:
            rm = update_replay_memory(rm, my_id, dest_id, prev_router_id, prev_action, queueing_delay, done, is_dead, free_vcs)
